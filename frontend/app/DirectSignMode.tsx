...
async function withdrawWithDirectSign(_params: {
  ...
  // TODO: implement — see direct_sign_utils.ts for the full design.
...
export function DirectSignMode({ selectedDWallet }: Props) {
  ...
  async function handleWithdraw() {
    ...
    setStatus(null);
    try {
      const withdrawResult = await withdrawWithDirectSign({
    ...
    return (
      <Flex direction="column" gap="3">
        <Callout.Root color="green">
          ...
          <Callout.Text>Withdrawal signed and submitted</Callout.Text>
        </Callout.Root>
        <Flex direction="column" gap="2" p="3" style={{ background: "var(--gray-a3)", borderRadius: "var(--radius-3)" }}>
          ...
          {result.signId && (
            <Box>
              ...
              <Code size="1" style={{ wordBreak: "break-all", display: "block" }}>{result.signId}</Code>
            </Box>
          )}
        </Flex>
  ...
  return (
    <Flex direction="column" gap="3">
      ...
      <Button size="3" onClick={handleWithdraw} disabled={pending}>
        {pending ? <><Spinner />Withdrawing...</> : "Direct Sign & Withdraw"}
      </Button>
    </Flex>
  );
}
